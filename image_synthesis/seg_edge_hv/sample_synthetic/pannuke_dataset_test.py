...
class MyDataset(Dataset):
    def __init__(self, syn_set):
        # self.data = []
        # with open('./setup_pathology_ds/synthetic/pannuke_test.json', 'rt') as f:
        #     for line in f:
        #         self.data.append(json.loads(line))

        self.syn_set = syn_set

        self.pannuke_data_path = f'/Dataset/pannuke/splits/MICCAI2024_synthetic/{syn_set}'
        self.img_dir = f'/Dataset/pannuke/splits/images/test' if 'test' in syn_set \
                    else f'/Dataset/pannuke/splits/images/val'
        self.img_pths = sorted(glob.glob(os.path.join(self.img_dir, "*.png")))
        self.cls_pths = sorted(glob.glob(os.path.join(self.pannuke_data_path, "classes", "*.png")))
        self.ist_pths = sorted(glob.glob(os.path.join(self.pannuke_data_path, "instances", "*.png")))
        self.dst_pths = sorted(glob.glob(os.path.join(self.pannuke_data_path, "dist_masks", "*.npy")))

        # print(len(self.img_pths))
        # print(len(self.cls_pths))
        # print(len(self.ist_pths))
        # print(len(self.dst_pths))
        # exit()
        self.num_classes = 6
