...
class MyDataset(Dataset):
    def __init__(self):
        self.data = []
        with open('./setup_pathology_ds/v3/pannuke_train.json', 'rt') as f:
            for line in f:
    ...
    def __getitem__(self, idx):
        ...
        hv_fn = os.path.join(self.pannuke_data_path, 'dist_masks/train', fn_npy)

        prompt_base = item['prompt']
        tissue_type = prompt_base.split("high quality histopathology ")[-1].split(" tissue nuclei image")[0]
        
        ## TEMP ##
        # item['prompt'] = "High quality histopathology colon tissue image"

        # prompt = item['prompt']

        # cls = cv2.imread(self.pannuke_data_path + cls_fn)
        # ist = cv2.imread(self.pannuke_data_path + ist_fn)
    ...
    def set_prompt(self, cls, tissue_type):
        prompt = f"high quality histopathology H&E-stained {tissue_type} tissue nuclei image"
